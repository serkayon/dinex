...
const getPending = (row) => Math.max(0, (Number(row?.target) || 0) - getProduced(row));
  ...
  return (
    <div className="w-full bg-[#F4F7FB] p-3 md:p-5 min-h-screen">
      <div className="bg-white rounded-2xl border border-[#e9edf5] shadow-sm p-4 md:p-5">
                  ...
                  <table className="w-full min-w-[900px] border-collapse">
                    <thead>
                      <tr className="bg-white">
                        {["Split", "Hour", "Model", "Target", "OK", "Rework", "Reject", "Produced", "Pending", "OEE"].map((head) => (
                    ...
                    <tbody>
                      {group.rows.map((item) => (
                        <tr key={`${group.shift}-${item.splitNo}`} className="border-b border-[#edf2f7] last:border-b-0">
                          ...
                          <td className="px-3 py-2 text-sm font-semibold text-[#d97706]">{getPending(item)}</td>
        ...
        {(period === "7d" || period === "30d" || period === "custom") ? (
          <div className="mt-5 rounded-2xl border border-[#e8edf5] overflow-hidden">
            <div className="px-4 py-3 bg-[#f4f8f6] border-b border-[#e8edf5] flex items-center justify-between">
              <h2 className="text-sm md:text-base font-bold text-[#22543d]">
                {period === "7d"
                  ? "Last 7 Days - Day-wise Summary"
                  : period === "30d"
                    ? "Last 1 Month - Day-wise Summary"
                    : "Custom Range - Day-wise Summary"}
              </h2>
            ...
            <div className="overflow-auto">
              <table className="w-full min-w-[980px] border-collapse">
                <thead>
                  <tr className="bg-white">
                    {["Date", "Shifts", "Incharges", "Avg Manpower", "Target", "OK", "Rework", "Reject", "Produced", "Pending", "Day OEE", "View Details"].map((head) => (
                      <th key={head} className="px-3 py-3 text-left text-xs font-bold uppercase tracking-wide text-slate-500 border-b border-[#edf2f7]">
                        {head}
                      </th>
                    ))}
                ...
                <tbody>
                  {dayWiseRows.length === 0 ? (
                    <tr>
                      <td colSpan={12} className="px-3 py-8 text-center text-sm text-slate-500">No finished shifts for selected dates.</td>
                    </tr>
                  ) : (
                    dayWiseRows.map((item) => {
                      const produced = item.ok + item.rework + item.reject;
                      const pending = Math.max(0, item.target - produced);
                      ...
                      return (
                        <tr key={item.date} className="border-b border-[#edf2f7] last:border-b-0">
                          ...
                          <td className="px-3 py-2 text-sm font-semibold text-[#d97706]">{pending}</td>
                ...
                <tbody>
                  {buildSplitDetails(selectedShiftDetail).map((row) => (
                    <tr key={`detail-${row.splitNo}`} className="border-b border-[#edf2f7] last:border-b-0">
                      ...
                      <td className="px-3 py-2 text-sm font-semibold text-[#d97706]">{getPending(row)}</td>
